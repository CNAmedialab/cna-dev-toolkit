...
# 建立獨立的 source file
CNA_RC="$HOME/.cna-toolkit.sh"
cat > "$CNA_RC" <<EOF
# CNA Dev Toolkit Configuration
# Generated by install.sh on $(date '+%Y-%m-%d %H:%M:%S')

# Toolkit directory
export CNA_TOOLKIT_DIR="$TOOLKIT_DIR"

# Aliases
alias newproject='bash ~/.cursor/scripts/newproject.sh'
alias council-query='bash \$CNA_TOOLKIT_DIR/scripts/council-query.sh'

# Issue helpers (載入函數)
source \$CNA_TOOLKIT_DIR/scripts/issue-helpers.sh 2>/dev/null || true
EOF

success "配置已寫入 $CNA_RC"

# 檢查 .zshrc 是否已載入
if grep -q ".cna-toolkit.sh" "$SHELL_RC" 2>/dev/null; then
  warning "Shell 配置已存在，跳過"
else
  cat >> "$SHELL_RC" <<'EOF'

# CNA Dev Toolkit
[ -f ~/.cna-toolkit.sh ] && source ~/.cna-toolkit.sh
EOF
  success "已加入 source 到 $SHELL_RC"
fi
